...
@app.get("/posts", response_model=List[schemas.Post])  #get is the method; "/" is the path
def get_posts(db: Session = Depends(get_db)):    #get_posts is a function
    posts = db.query(models.Post).all()     #query is performing SQL
    return posts
...
@app.get("/posts/{id}", response_model=schemas.Post) #{id} represents a path parameter
