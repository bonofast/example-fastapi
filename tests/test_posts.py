...
def test_get_all_posts(authorized_client, test_posts):
    ...
    assert res.status_code == 200   # changed this one from 200
...
def test_create_post_defatul_published_true(authorized_client, test_user, test_posts):
...
def test_unauthorized_user_create_post(client, test_user, test_posts):
    res = client.get("/posts", json={"title": "arbitrary title", "content": "arbitrary content"})
    assert res.status_code == 401
...
def test_udpdate_post(authorized_client, test_user, test_posts):
    data = {
        "title": "updated title",
